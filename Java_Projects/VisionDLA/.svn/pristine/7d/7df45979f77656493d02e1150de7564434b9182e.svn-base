...
public class Utility {
	...
	/**
		 ...
		 */
	public static VisionDlaPersistence getVisionDlaPersistenceLocal() throws VisionDlaException{
		String visionDlaPersistenceJNDI = "visiondla/VisionDlaPersistenceImpl/local";
  	Object robj = null;
  	try {
			if(ctx == null ){
			  ctx=new InitialContext();
			}
			robj = ctx.lookup( visionDlaPersistenceJNDI);
		} catch (NamingException e) {
			logger.error("getVisionDlaPersistenceLocal(): Failed to get VisionDlaPersistence local EJB", e);
			throw new VisionDlaException("Failed to get VisionDlaPersistence local EJB-NamingException", e);
		} 
  	logger.info("getVisionDlaPersistenceLocal(): Class name->" + robj.getClass().getName());
  	return (VisionDlaPersistence)robj;
//		return new VisionDlaPersistenceImpl();
	}
