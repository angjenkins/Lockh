...
from configs.commonconfigs import RelativePathToHashesFile,TokenTimeout
...
def does_authenticate(username,password,p3apisalt,remote_addr):
    hashes = loadHashes()
    ...
    if username in GLOBAL_BAD_LOGIN:
        ...
        if (count >= LIMIT_NUMBER_BAD_LOGINS):
            ...
            LogActivity.logTooManyLoginAttempts(username,remote_addr)
            return False;

    if username not in hashes:
        LogActivity.logBadCredentials(username,remote_addr)
...
def update_acsrf(session_id,remote_addr):
    acsrf = GLOBAL_SESSION_DICT[session_id][0];
    return update_new_acsrf(session_id,acsrf,remote_addr)

def update_new_acsrf(session_id,acsrf,remote_addr):
    timestamp = datetime.datetime.now();
    GLOBAL_SESSION_DICT[session_id] = [acsrf,timestamp]
    LogActivity.logDebugInfo("SETTING ACSRF session, acsrf "+session_id+"."+GLOBAL_SESSION_DICT[session_id][0],remote_addr)
    return session_id;
...
def is_valid_acsrf(session_id,acsrf,remote_addr):
    if (session_id in GLOBAL_SESSION_DICT):
        timestamp = GLOBAL_SESSION_DICT[session_id][1]
        timenow = datetime.datetime.now()
        timedelta = timenow - timestamp
        if (timedelta < datetime.timedelta(seconds=TokenTimeout)):
            if (acsrf != GLOBAL_SESSION_DICT[session_id][0]):
                LogActivity.logDebugInfo("ACSRF Mismatch provided vs. stored :"+acsrf+","+GLOBAL_SESSION_DICT[session_id][0],remote_addr)
                return False
            else:
                return True
        else:
            LogActivity.logTimeout(session_id,remote_addr)
            return False
    else:
        LogActivity.logMissingSession(session_id,remote_addr)
        return False;
...
def del_session(session_id,remote_addr):
    obj = (GLOBAL_SESSION_DICT.pop(session_id, None))
    if session_id in GLOBAL_SESSION_DICT:
        LogActivity.logMissingSession(str(session_id)+"failed to remove",remote_addr)
    else:
        LogActivity.logMissingSession(str(session_id)+"removed",remote_addr)
